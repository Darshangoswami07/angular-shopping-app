...
@Component({
  selector: 'app-trending-products', standalone: true, imports: [CommonModule, RouterLink, ScrollRevealDirective],
  template: `
    <section id="shop" class="py-20 px-4 bg-slate-50"><div class="container mx-auto">
      ...
      <div *ngIf="!loading && products.length" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
      ...
      <div *ngIf="!loading && !products.length" class="text-center">
        <p class="text-slate-600">{{ loadFailed ? "Products are taking longer than usual to load." : "No products are available yet." }}</p>
...
export class TrendingProductsComponent implements OnInit {
  products: Product[] = []; loading = true; loadFailed = false; stars = [1, 2, 3, 4, 5]; skeletons = [1, 2, 3, 4];
  constructor(
    ...
    this.productService
      ...
      .subscribe({
        ...
        error: () => {
          ...
          this.toast.error('Products unavailable', 'Please try again shortly.');
        }
      });
  }
  ...
  addToCart(product: Product) {
    ...
    this.cartService.addToCart(product.id).subscribe({ next: () => { this.cartService.openCart(); this.toast.success('Added to Cart', product.name); }, error: err => this.toast.error('Unable to add to cart', err.error?.message || 'Please sign in and try again.') });
  }
}
